 ...
 * - Sí: Login con usuario y contraseña (conecta con backend)
 * - No: Formulario de registro completo (conecta con backend)
...
export class Profile implements OnInit, OnDestroy {
  ...
  /**
   * Procesa el login conectando con el backend
   */
  onLogin(): void {
    ...
    this.authService.login(credentials).subscribe({
      next: (response) => {
        ...
        if (response.success) {
          // Login exitoso - crear sesión en memoria con datos del backend
          this.userSession = {
            username: response.username,
            profileImage: response.profileImage || 'img/quetzalcoatl.png',
          ...
          console.log('[Profile] Login exitoso:', response.username);
        }
      },
      error: (error) => {
        this.isLoading = false;
        this.loginError = error?.error?.message || 'Error al iniciar sesión. Intenta de nuevo.';
        console.error('[Profile] Error en login:', error);
      }
    });
  }
  ...
  /**
   * Procesa el registro conectando con el backend
   */
  onRegister(): void {
    ...
    this.authService.register(registerData).subscribe({
      next: (response) => {
        ...
        if (response.success) {
          ...
          // Crear sesión en memoria con datos del backend
          this.userSession = {
            username: response.username,
            profileImage: response.profileImage || 'img/quetzalcoatl.png',
            learningTime: response.learningTime,
          ...
          setTimeout(() => {
            this.modalState = 'none';
            this.startLearningTimer();
            console.log('[Profile] Registro exitoso:', response.username);
          }, 1500);
        }
      },
      error: (error) => {
        this.isLoading = false;
        this.registerError = error?.error?.message || 'Error al registrarse. Intenta de nuevo.';
        console.error('[Profile] Error en registro:', error);
      }
    });
  }
